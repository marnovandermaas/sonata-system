...
#ifndef __DV_DPI_SPIDEVDPI_H_
#define __DV_DPI_SPIDEVDPI_H_
#include <stdint.h>
...
// single device per bus. Therefore, at present, the SPI device models derive from `spidevdpi`
// directly.
//
// If multiple devices are required to share a single bus then a `spi_device` base class may be
// introduced and an instance of `spidevdpi` will handling the mapping from `cs` line to `spi_device`
// object.
class spidevdpi {
public:
  spidevdpi(unsigned dataw,    // Number of data lines.
