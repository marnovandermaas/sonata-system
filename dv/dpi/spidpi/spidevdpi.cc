...
void spidevdpi::sampleEdge(uint32_t cs, uint32_t copi, uint32_t oobIn) {
...
uint32_t spidevdpi::launchEdge(uint32_t cs, uint32_t oobIn) {
  unsigned cipo = 1;
  // Suppress traffic if CS is high because the current SPI model ignores CS.
  if (cs & 1u) {
    outBits = 0u;
...
void *spidevdpi_create(const char *id,      // Bus identification.
  ...
  if (!strcmp(id, "flash")) {
    ...
    ctx = new spidevdpi(dataW, oobInW, oobOutW, true);
...
void spidevdpi_destroy(void *ctx_v) {
  spidevdpi *ctx = (spidevdpi*)ctx_v;
  assert(ctx);
  delete ctx;
}

// Sampling transition on the SCK line.
void spidevdpi_sampleEdge(void *ctx_v, uint32_t cs, uint32_t copi, uint32_t oobIn) {
  spidevdpi *ctx = (spidevdpi*)ctx_v;
  assert(ctx);
...
uint32_t spidevdpi_launchEdge(void *ctx_v, uint32_t cs, uint32_t oobIn) {
  spidevdpi *ctx = (spidevdpi*)ctx_v;
  assert(ctx);
...
void spidevdpi_csEdge(void *ctx_v, uint32_t cs, uint32_t oobIn) {
  spidevdpi *ctx = (spidevdpi*)ctx_v;
  assert(ctx);
